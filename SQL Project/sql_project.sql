...
/* If a member has played in court 1 and court 2, then an entry is disaplyed
   for each */
...
/* Note: If a member has booked more than once on a given day, each booking cost
  is calculated separately. The results are not added, which is what the
  question asks. */
...
SELECT f.name as facility, CONCAT(firstname, ' ',surname) AS member,
	...
	INNER JOIN Members
		ON Bookings.memid = Members.memid
	INNER JOIN (SELECT * FROM Facilities)f
